	  ...
	  graph_{std::move(graph)}
{
	...
	steps_.resize(graph_.nodes.size());
}
...
void GraphGui<T>::OnClick(bwgui::Point<int> point)
{
	...
	if (index == -1) return;
	int i = 0;
	graph_.traverse(index, [this, &i](int from, int to){ steps_[i] = {from, to}; ++i; });
	view_.reset_styles();
	next_ = 0;
}
...
void GraphGui<T>::OnLoop()
{
	...
	view_.vertexes[to].style.border = bwgui::Yellow;
	view_.vertexes[to].style.background = bwgui::Green;
	if (from > -1)
	{
